...
namespace TreeViewer.ViewModels {
	public class TreeDiffControlViewModel : ViewModelBase {
		...
		public TreeDiffControlViewModel() {
			...
			SearchCommand = new RelayCommand(SearchCommandBinding_Executed, new Predicate<object>((s) => LeftFsmTree.RootNode != null));
			OpenCommand = new RelayCommand(OpenCommandBinding_Executed);

			LeftFsmTree = new FsmTreeViewModel();
			RightFsmTree = new FsmTreeViewModel();
		}
		...
		void loadFileWorker_DoWork(object sender, DoWorkEventArgs e) {
			...
			try {
				using (FileStream stream = new FileStream(arg.Path, FileMode.Open)) {
					e.Result = new TreenodeViewModel(Treenode.Read(stream), null);
				}
